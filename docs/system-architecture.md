...
5. `prepare_proxy` — `proxy_pkg.prepare` runs the Swift tool + a caller-supplied step in between:
   ...
   c. `enrich_lockfile_products` — Validates per-project version and invalidates stale `products[]` if `spm_cache_version` doesn't match the running `SPMCache::VERSION` (prevents downstream from using product metadata written by buggy pre-fix code). Then runs `swift package describe` per checkout → writes `products[]` into `spm-cache.lock` (idempotent, skips entries already enriched); if `describe` comes back with no products or fails outright (e.g. a local-path binaryTarget whose artifact isn't present in the checkout copy), falls back to regex-parsing only `.library(name:)` declarations straight out of the checkout's `Package.swift` text — a `.binaryTarget` is never a product on its own, so it's never scanned
...
**`GenProxy`**: Loads lockfile (merging `dependencies` across projects, same as `GenUmbrella`) → `ProxyGenerator` iterates packages, skips plugin-only ones (emitting a `plugin`-status `graph.json` entry instead) *and* skips a package proven transitive-only (`PackageRef.isTransitiveOnly`) — no sub-proxy folder, no root-proxy dependency, no `graph.json` entry. This matters here even more than in the umbrella: the root proxy is what actually gets wired into the real Xcode project, so referencing a transitive-only package's own sub-proxy (which independently pins it) reproduces the exact version-conflict UmbrellaGenerator avoids, but baked into the real project's package graph instead of spm-cache's internal checkout step. Otherwise expands each remaining package's real library products (`Lockfile.PackageRef.libraryProducts`, with a single-entry legacy fallback when `products[]` metadata is absent) and checks `BinariesCache` per product name, generating one proxy folder per package (all its products in one manifest) + root proxy + `graph.json`.
