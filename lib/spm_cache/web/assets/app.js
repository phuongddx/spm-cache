...
(() => {
  ...
  // 16-05-port: `<thead>`, the filter/chip toolbar, and the sync bar are
  // now static markup (rewiring notes section 7) — renderState writes
  // ONLY into `#state-rows` (the `<tbody>`). `lastPackages` is the
  // RENDER cache of the last successful /api/state fetch that the
  // filter/chip handlers re-render from without a new request (CP10
  // client-side: the fetch itself is never cached, only its result).
  let lastPackages = [];
  let stateFilterText = '';
  let stateFilterChip = 'all'; // 'all' | 'hit' | 'missed' | 'other'

  const packageVisible = (p) => {
    if (stateFilterText && !p.name.toLowerCase().includes(stateFilterText)) return false;
    if (stateFilterChip === 'hit') return p.state === 'hit';
    if (stateFilterChip === 'missed') return p.state === 'missed';
    if (stateFilterChip === 'other') return p.state !== 'hit' && p.state !== 'missed';
    return true;
  };

  const updateCacheBadge = (pendingCount) => {
    const badge = byId('rail-badge-cache');
    badge.hidden = pendingCount === 0;
    badge.textContent = String(pendingCount);
    badge.title = `${pendingCount} unapplied cache change${pendingCount === 1 ? '' : 's'}`;
  };

  // D-05/D-06: the bar's OWN visibility/message are patched on the
  // static node every render — never inserted/removed (context
  // decision 4). A freshly-shown bar always starts with a blank
  // message slot (the prior create-fresh-every-poll behavior, kept
  // byte-identical): any in-flight Apply/Revert feedback is replaced
  // by the next successful poll, exactly as the old buildSyncBar() did.
  const updateSyncBar = (pendingCount) => {
    byId('state-sync-bar').hidden = pendingCount === 0;
    saySync('');
  };

  const renderStateRows = () => {
    const tbody = byId('state-rows');
    tbody.replaceChildren();
    let shown = 0;
    lastPackages.forEach((p) => {
      if (!packageVisible(p)) return;
      shown += 1;
      tbody.append(stateRow(p));
    });
    // D-09/A5: #state-empty is the FILTER-yields-zero-rows indicator —
    // a genuinely empty cache (lastPackages.length === 0) renders an
    // honestly empty table instead (no fabricated "no matches" copy).
    byId('state-empty').hidden = !(lastPackages.length > 0 && shown === 0);
    byId('chip-n-all').textContent = String(lastPackages.length);
    const hit = lastPackages.filter((p) => p.state === 'hit').length;
    const missed = lastPackages.filter((p) => p.state === 'missed').length;
    const totalBytes = lastPackages.reduce(
      (sum, p) => sum + (Number.isFinite(p.size_bytes) ? p.size_bytes : 0), 0,
    );
    byId('state-total').textContent = `${hit} hit · ${missed} missed · ${humanBytes(totalBytes)} on disk`;
  };
  ...
  const renderState = (envelope) => {
    ...
    const pendingCount = lastPackages.filter((p) => p.pending).length;
    updateSyncBar(pendingCount);
    updateCacheBadge(pendingCount);
  };

  byId('state-filter').addEventListener('input', (e) => {
    stateFilterText = e.target.value.toLowerCase();
    renderStateRows();
  });

  Array.prototype.forEach.call(document.querySelectorAll('.chip-group .chip'), (chipEl) => {
    chipEl.addEventListener('click', () => {
      Array.prototype.forEach.call(document.querySelectorAll('.chip-group .chip'),
        (other) => other.classList.remove('is-on'));
      chipEl.classList.add('is-on');
      stateFilterChip = chipEl.dataset.state;
      renderStateRows();
    });
  });
  ...
  // 16-05-port: check rows move into `#check-list` (a `<ul>`) and the
  // tallies into `#doctor-summary` — both persistent static siblings
  // of the cold-load `.loading` placeholder (rewiring notes section 7).
  // Doctor is on-demand only (never auto-polled), so a `renderEmpty()`
  // full-body replace would permanently orphan those two persistent
  // targets the very first time a real Run Doctor click landed
  // has_run: true — the `.loading` node is patched/removed instead,
  // exactly like the graph panel's own cold-load pattern (decision 6).
  const updateDoctorBadge = (summary) => {
    const badge = byId('rail-badge-doctor');
    if (!summary || (!summary.failures && !summary.warnings)) { badge.hidden = true; return; }
    const failing = summary.failures > 0;
    badge.hidden = false;
    badge.className = `rail-status ${failing ? 'rail-status-fail' : 'rail-status-warn'}`;
    badge.textContent = failing ? '✗' : '!';
    badge.title = failing
      ? `${summary.failures} check${summary.failures === 1 ? '' : 's'} failing`
  ...
  const renderDoctor = (envelope) => {
    ...
    // Set on ANY successful fetch (including has_run: false) — the
    // persistent `.loading`/`#doctor-summary`/`#check-list` nodes must
    // never be discarded, so a LATER fetch failure has to prepend its
    // error line rather than fall through renderPanelError's
    // first-ever-failure replaceChildren path (which would otherwise
    // orphan those static containers permanently).
    body.dataset.rendered = '1';
    if (!data.has_run) {
      byId('doctor-stamp').textContent = '';
      if (loading) loading.textContent = 'Doctor has not run yet. Select Run Doctor to check your environment.';
      byId('doctor-summary').hidden = true;
      byId('check-list').replaceChildren();
      updateDoctorBadge(null);
      return;
    }
    byId('doctor-stamp').textContent =
    ...
    data.checks.forEach((check) => {
      const li = el('li', { class: `check-row check-row-${check.status}` });
      li.append(el('span', {
        class: `check-marker check-marker-${check.status}`,
        text: MARKER[check.status] || '·',
      }));
      const head = el('div', { class: 'check-head' });
      head.append(el('span', { class: 'check-name', text: check.name, title: check.name }));
      head.append(el('span', { class: 'check-message', text: check.message, title: check.message }));
      li.append(head);
      if (check.status !== 'ok' && check.fix_hint) {
        const hint = el('p', { class: 'fix-hint' });
        hint.append(el('span', { class: 'fix-hint-label', text: 'fix' }));
        hint.append(el('span', { class: 'cmd', text: check.fix_hint }));
        li.append(hint);
      }
      list.append(li);
    });
    const summary = data.summary || {};
    const summaryEl = byId('doctor-summary');
    summaryEl.replaceChildren();
    [['fail', '✗', summary.failures, 'failed'], ['warn', '!', summary.warnings, 'warnings'],
      ['ok', '✓', summary.ok, 'passed']].forEach(([key, glyph, count, word]) => {
      const tally = el('span', { class: `tally tally-${key}` });
      tally.append(el('span', { class: 'glyph', text: glyph }));
      tally.append(el('b', { class: 'mono', text: String(count) }));
      tally.append(` ${word}`);
      summaryEl.append(tally);
    });
    summaryEl.append(el('span', { class: 'doctor-summary-note', text: `${data.checks.length} checks` }));
    summaryEl.hidden = false;
    updateDoctorBadge(summary);
  };
  ...
  // -- 9b. nav-rail surface switching (app-shell port, 2026-09) ---------
  // Ported from the redesign mockup's own real DOM logic (zero mock
  // data) — click, digit keys, `/` and `Escape`, persisted in
  // localStorage. Exactly one of the four `.surface` sections is
  // visible at a time; Run Log ships `is-active`/visible by default.
  const SURFACE_KEY = 'spm-cache.surface';
  const SURFACES = ['run', 'cache', 'doctor', 'graph'];
  const railItems = Array.prototype.slice.call(document.querySelectorAll('.rail-item'));

  const showSurface = (name) => {
    if (SURFACES.indexOf(name) === -1) return;
    railItems.forEach((b) => {
      const on = b.dataset.surface === name;
      b.classList.toggle('is-active', on);
      if (on) b.setAttribute('aria-current', 'page'); else b.removeAttribute('aria-current');
    });
    Array.prototype.forEach.call(document.querySelectorAll('.surface'), (s) => {
      const on = s.dataset.surface === name;
      s.classList.toggle('is-active', on);
      s.hidden = !on;
    });
    try { localStorage.setItem(SURFACE_KEY, name); } catch (e) { /* private mode */ }
  };

  railItems.forEach((b) => {
    b.addEventListener('click', () => showSurface(b.dataset.surface));
  });
  ...
  document.addEventListener('keydown', (ev) => {
    ...
    if (!typing && ev.key >= '1' && ev.key <= '4') {
      showSurface(SURFACES[Number(ev.key) - 1]);
    }
    if (!typing && ev.key === '/') { ev.preventDefault(); showSurface('cache'); byId('state-filter').focus(); }
    if (ev.key === 'Escape' && tag === 'INPUT') ev.target.blur();
  });
  ...
  const boot = () => {
    ...
    try { savedSurface = localStorage.getItem(SURFACE_KEY); } catch (e) { savedSurface = null; }
    showSurface(SURFACES.indexOf(savedSurface) !== -1 ? savedSurface : 'run');
    const loop = async () => {
  ...
  // fresh (never cached in `ctl`) since #sync-apply is a static node
  // whose disabled state this is the ONLY writer of (16-05-port: the
  // node is no longer created/destroyed per render, so no re-apply is
  // needed on show — the DOM already remembers the frozen state).
  const freeze = (on) => {
  ...
  // 16-05-port: the bar, its honesty sentence, and its two buttons are
  // now static markup (context decision 4) — this section only owns
  // the click handlers and the `#sync-message` feedback slot, never
  // `msg` (that one belongs to the run/Run Log controls row).
  ...
  // Static nodes from page load (context decision 4) — bind once.
  // Revert all is NOT part of the freeze set (A5 names four controls,
  // not five; A3: Revert is not slot-gated).
  byId('sync-revert').addEventListener('click', clickRevert);
  byId('sync-apply').addEventListener('click', clickApply);
