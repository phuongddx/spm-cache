...
(() => {
  ...
  // W3 (14-UI-REVIEW → 15 polish): the pills are PERSISTENT nodes —
  // created on first need, then patched (hidden + label) — so the
  // overlay is never wholesale replaced and keyboard focus survives
  // every queued line while paused.
  let pauseBtn = null;
  let filterBtn = null;
  const renderPill = () => {
    if (!replaying && !follow && pending >= 1) {
      if (!pauseBtn) {
        pauseBtn = el('button', { type: 'button', class: 'log-pill-btn', text: COPY.paused(pending) });
        pauseBtn.addEventListener('click', resumeFollow);
        overlay.append(pauseBtn);
      }
