...
(() => {
  ...
  const renderPill = () => {
    ...
    if (activeFilter) {
      // D-09: ONE button carrying 'filtered: {name}'; activating it
      // clears the filter with the view put — no scroll change. The
      // label doubles as the tooltip for the 240px ellipsis.
      const fp = el('button', {
        type: 'button',
        class: 'log-pill-btn log-filter-pill',
        text: COPY.filtered(activeFilter.name),
        title: COPY.filtered(activeFilter.name),
      });
      fp.addEventListener('click', clearFilter);
      overlay.append(fp);
    }
  };
  ...
  const renderChips = () => {
    ...
    railPhases.querySelectorAll('.log-chip').forEach((chip) => chip.remove());
    railPackages.querySelectorAll('.log-chip').forEach((chip) => chip.remove());
    anchors.forEach((anchor) => {
      const isActive = activeFilter
        && activeFilter.kind === anchor.kind && activeFilter.name === anchor.name;
      const chip = el('button', { type: 'button', class: 'log-chip', text: anchor.name, title: anchor.name });
      chip.setAttribute('aria-pressed', isActive ? 'true' : 'false');
      if (isActive) chip.classList.add('log-chip-active');
  ...
  const renderSwitchNotice = (previousRun) => {
    ...
    const control = el('button', {
      type: 'button',
      class: 'log-pill-btn log-runid-btn',
      text: previousRun,
      title: previousRun, // run ids inside notices ellipsize with tooltips (long-text row)
    });
    control.addEventListener('click', () => loadRun(previousRun));
    switchBar.replaceChildren(
      el('span', { class: 'log-switch-text', text: COPY.switchNotice }),
      control,
    );
  };
