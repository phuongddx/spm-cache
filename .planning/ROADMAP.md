...
- [x] 12-05-PLAN.md — Watch per-cycle run logs via the installer_factory wrapper + D-08 no-allowlist proof (D-09)
...
- [x] 13-01-PLAN.md — Tracer: WEBrick skeleton + security middleware (Host/Origin/token) + `Command::Web` lifecycle (port probe, marker relaunch, signals, browser open) + /api/graph slice
...
- [x] 13-03-PLAN.md — Offline frontend assets per UI-SPEC: index.html + styles.css + app.js + vendored cytoscape (token bootstrap, three panels)
