...
- `lib/spm_cache/command/watch.rb` (56 lines) — `spm-cache watch` subcommand with `--once` and `--debounce=SECONDS`.
- `spec/watch_spec.rb` (137 lines) — 9 specs (12 reported including 3 spec_helper.rb globals) covering file watching, run_once, change detection, continue-on-error, missing project, debounce, flag parsing. All passing.

## Design note
The spec proposed native FSEvents via Fiddle; the existing `use --watch` already used portable mtime polling successfully. Phase 5 adds a dedicated `Core::Watcher` alongside the retained `use --watch` loop (command/use.rb:50-80) + `watch` command with the full feature set (debounce, --once, continue-on-error), keeping the zero-dependency polling approach since it's proven and macOS stdlib-sufficient. AUTO-05 ("no new gem dependency") is satisfied.
