...
- [x] `spec/web_jobs_spec.rb` — spawn shape (argv/cwd/env/pgroup/detach/null-stdio via fake-bin side effects), slot atomicity, derive-based release, 409
- [x] Fake-bin fixture script (writes a run log with header+run_end; `sleep` mode for in-flight integration row)
- [x] `spec/web_build_routes_spec.rb` — token/verb/body/409/lock-snapshot matrix (13-04 posture)
- [x] `spec/installer_rollback_lock_spec.rb` — order-before-rm_rf, release-on-raise, byte-exact announce
- [x] `spec/run_log_trigger_spec.rb` — env normalization → header trigger
- [x] Extend `spec/web_integration_spec.rb` (+ `spec/support/web_server_boot.rb` as needed) — POST row + shutdown-with-in-flight-build exit-0 assertion
...
- [x] Manual-only table executed and recorded (7 rows above, D-14 pattern)

**Approval:** validated 2026-09-02 — 8/8 map rows green (289 examples, 0 failures, scoped re-run); all 7 manual rows executed 2026-09-02 (agent-browser) and recorded in 15-06-SUMMARY.md.
...
- All Per-Task Map rows re-run scoped on 2026-09-02: `bundle exec rspec spec/web_jobs_spec.rb spec/web_build_routes_spec.rb spec/installer_rollback_lock_spec.rb spec/run_log_trigger_spec.rb spec/command_build_rebuild_spec.rb spec/web_integration_spec.rb spec/web_frontend_spec.rb` → **289 examples, 0 failures (33.5 s)**.
- Every Wave 0 artifact exists on disk (four new spec files, `spec/fixtures/fake_spm_cache_bin.rb`, and the integration rows `describe 'POST /api/build (BLD-01 tracer)'` + `describe 'shutdown with an in-flight UI build (D-02/WEB-03, P5 as a spec)'`) → `wave_0_complete: true`.
- All 7 manual rows executed 2026-09-02 via the agent-browser probe and recorded row-by-row in 15-06-SUMMARY.md; BLD-01..BLD-04 each additionally carry automated evidence → `nyquist_compliant: true`.
