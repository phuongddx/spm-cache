...
RSpec.describe SPMCache::SPM::Buildable do
  ...
  describe '#xcodebuild low deployment target retry' do
    ...
    # WR-04: on logged runs the raised message carries only the 60-line
    # failure_detail tail -- a diagnostic that scrolls past it (large
    # parallel builds print hundreds of post-failure lines) silently
    # stopped the recovery from firing, making build outcomes differ
    # between --no-run-log and logged runs. The retry must also match the
    # full streamed content that Core::Sh attaches to the error.
    it 'retries when the diagnostic is only in the full streamed output, past the message tail (WR-04)' do
      buried_error = SPMCache::Core::GeneralError.new(
        "Command failed (exit 65): xcodebuild build ...\n#{Array.new(80) { |i| "filler line #{i}" }.join("\n")}"
      )
      buried_error.full_output =
        "FSPageControl.swift:105:15: error: 'layoutSublayers(of:)' is only available in iOS 10.0 or newer"
      call_count = 0
      allow(SPMCache::Core::Sh).to receive(:run) do |cmd, _opts|
        call_count += 1
        raise buried_error if call_count == 1
    ...
    it 'does not retry and re-raises for an unrelated build failure' do
      ...
      expect do
        buildable.xcodebuild('platform=iOS Simulator,name=iPhone 17', derived_data_path: '/dd')
      end.to raise_error(SPMCache::Core::GeneralError, /unrelated compile error/)
...
RSpec.describe SPMCache::SPM::Package do
  describe 'DEFAULT_DESTINATIONS' do
    it 'includes both simulator and device' do
      expect(described_class::DEFAULT_DESTINATIONS).to eq(%w[iphonesimulator iphoneos])
    end
  end
end
